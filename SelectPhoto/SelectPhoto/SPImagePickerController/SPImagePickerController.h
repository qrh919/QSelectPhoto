...
@class SPAlbumCell, SPAssetCell;
@protocol SPImagePickerControllerDelegate;
@interface SPImagePickerController : UINavigationController
...
#pragma mark - 自定义
/// 添加、删除资源对象
@property (nonatomic, copy) void (^getCurrentChangeAsset)(BOOL isAdd,SPAssetModel *model);
...
@property (nonatomic, strong) NSMutableArray<SPAssetModel *> *selectedModels;
...
@property (nonatomic, weak) id<SPImagePickerControllerDelegate> pickerDelegate;

@end


@protocol SPImagePickerControllerDelegate <NSObject>
@optional
// The picker should dismiss itself; when it dismissed these handle will be called.
// You can also set autoDismiss to NO, then the picker don't dismiss itself.
// If isOriginalPhoto is YES, user picked the original photo.
...
// 你可以通过一个asset获得原图，通过这个方法：[[TZImageManager manager] getOriginalPhotoWithAsset:completion:]
// photos数组里的UIImage对象，默认是828像素宽，你可以通过设置photoWidth属性的值来改变它
- (void)imagePickerController:(SPImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto;
...
//- (void)imagePickerControllerDidCancel:(TZImagePickerController *)picker __attribute__((deprecated("Use -sp_imagePickerControllerDidCancel:.")));
...
// If user picking a gif image, this callback will be called.
// 如果用户选择了一个gif图片，下面的handle会被执行
- (void)imagePickerController:(SPImagePickerController *)picker didFinishPickingGifImage:(UIImage *)animatedImage sourceAssets:(PHAsset *)asset;
...
@interface SPAlbumPickerController : UIViewController
...
@interface SPImagePickerConfig : NSObject
...
@property (nonatomic, copy) void (^gifImagePlayBlock)(SPPhotoPreviewView *view, UIImageView *imageView, NSData *gifData, NSDictionary *info);
@end
