...
@property (nonatomic, strong) UIView *croveView;//选中的标识
...
- (void)setModel:(SPAssetModel *)model {
    ...
    if (self.useCachedImage && model.cachedImage) {
        ...
        int32_t imageRequestID = [[SPImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.sp_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
    ...
    if (model.needOscillatoryAnimation) {
        [UIView showOscillatoryAnimationWithLayer:self.selectImageView.layer type:SPOscillatoryAnimationToBigger];
    }
    ...
    self.showSelectCrove = model.mySelect;
}
...
- (void)setShowSelectBtn:(BOOL)showSelectBtn {
    ...
    BOOL selectable = [[SPImageManager manager] isPhotoSelectableWithAsset:self.model.asset];
...
-(void)setShowSelectCrove:(BOOL)showSelectCrove{
    _showSelectCrove = showSelectCrove;
    _croveView.hidden = !showSelectCrove;
}
...
- (void)setType:(TZAssetCellType)type {
    ...
    if (type == TZAssetCellTypeVideo) {
        ...
        _timeLength.sp_left = self.videoImgView.sp_right;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        if (![SPImagePickerConfig sharedInstance].showSelectedIndex && ![SPImagePickerConfig sharedInstance].showPhotoCannotSelectLayer) {
            [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:SPOscillatoryAnimationToBigger];
        }
        // 用户选中了该图片，提前获取一下大图
        [self requestBigImage];
    } else { // 取消选中，取消大图的获取
...
- (void)requestBigImage {
    ...
    _bigImageRequestID = [[SPImageManager manager] requestImageDataForAsset:_model.asset completion:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        [self hideProgressView];
...
- (UIImageView *)videoImgView {
    if (_videoImgView == nil) {
        ...
        [videoImgView setImage:[UIImage sp_imageNamedFromMyBundle:@"VideoSendIcon"]];
...
-(UIView *)croveView{
    if(!_croveView){
        _croveView = [[UIView alloc] initWithFrame:self.bounds];
        _croveView.backgroundColor = [[UIColor redColor] colorWithAlphaComponent:0.2];
        _croveView.layer.borderColor = [[UIColor redColor] colorWithAlphaComponent:0.8].CGColor;
        _croveView.layer.borderWidth = 1;
        [self.contentView addSubview:_croveView];
    }
    return _croveView;
}
...
- (void)layoutSubviews {
    ...
    _croveView.frame = self.bounds;
    if (self.allowPreview) {
        _selectPhotoButton.frame = CGRectMake(self.sp_width - 44, 0, 44, 44);
    } else {
        _selectPhotoButton.frame = self.bounds;
    }
    _selectImageView.frame = CGRectMake(self.sp_width - 27, 3, 24, 24);
    ...
    _imageView.frame = CGRectMake(0, 0, self.sp_width, self.sp_height);
    
    static CGFloat progressWH = 20;
    CGFloat progressXY = (self.sp_width - progressWH) / 2;
    _progressView.frame = CGRectMake(progressXY, progressXY, progressWH, progressWH);

    _bottomView.frame = CGRectMake(0, self.sp_height - 17, self.sp_width, 17);
    _videoImgView.frame = CGRectMake(8, 0, 17, 17);
    _timeLength.frame = CGRectMake(self.videoImgView.sp_right, 0, self.sp_width - self.videoImgView.sp_right - 5, 17);
    ...
    [self.contentView bringSubviewToFront:_croveView];
...
- (void)setModel:(SPAlbumModel *)model {
    _model = model;
    ...
    [[SPImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
- (void)layoutSubviews {
    [super layoutSubviews];
    _selectedCountButton.frame = CGRectMake(self.contentView.sp_width - 24, 23, 24, 24);
    NSInteger titleHeight = ceil(self.titleLabel.font.lineHeight);
    self.titleLabel.frame = CGRectMake(80, (self.sp_height - titleHeight) / 2, self.sp_width - 80 - 50, titleHeight);
...
@implementation SPAssetCameraCell
