...
- (void)viewDidLoad {
    ...
    if (self.needShowStatusBar) [UIApplication sharedApplication].statusBarHidden = YES;
}
...
- (void)configBarButtonItemAppearance {
    ...
    if (@available(iOS 9, *)) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[SPImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[SPImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<SPImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<SPImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<SPImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    _pushPhotoPickerVc = pushPhotoPickerVc;
    SPAlbumPickerController *albumPickerVc = [[SPAlbumPickerController alloc] init];
    ...
    if (self) {
        ...
        self.allowTakePicture = NO;//不允许拍照
        ...
        if (![[SPImageManager manager] authorizationStatusAuthorized]) {
            _tipLabel = [[UILabel alloc] init];
            _tipLabel.frame = CGRectMake(8, 120, self.view.sp_width - 16, 60);
            ...
            NSString *tipText = [NSString stringWithFormat:[NSBundle sp_localizedStringForKey:@"Allow %@ to access your album in \"Settings -> Privacy -> Photos\""],appName];
            _tipLabel.text = tipText;
            [self.view addSubview:_tipLabel];
            
            _settingBtn = [UIButton buttonWithType:UIButtonTypeSystem];
            [_settingBtn setTitle:self.settingBtnTitleStr forState:UIControlStateNormal];
            _settingBtn.frame = CGRectMake(0, 180, self.view.sp_width, 44);
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    SPPhotoPreviewController *previewVc = [[SPPhotoPreviewController alloc] init];
...
- (void)configDefaultSetting {
    ...
    CGFloat cropViewWH = MIN(self.view.sp_width, self.view.sp_height) / 3 * 2;
    self.cropRect = CGRectMake((self.view.sp_width - cropViewWH) / 2, (self.view.sp_height - cropViewWH) / 2, cropViewWH, cropViewWH);
}
...
- (void)setPhotoPreviewOriginDefImageName:(NSString *)photoPreviewOriginDefImageName {
    ...
    _photoPreviewOriginDefImage = [UIImage sp_imageNamedFromMyBundle:photoPreviewOriginDefImageName];
}

- (void)setPhotoOriginDefImageName:(NSString *)photoOriginDefImageName {
    _photoOriginDefImageName = photoOriginDefImageName;
    _photoOriginDefImage = [UIImage sp_imageNamedFromMyBundle:photoOriginDefImageName];
}

- (void)setPhotoOriginSelImageName:(NSString *)photoOriginSelImageName {
    _photoOriginSelImageName = photoOriginSelImageName;
    _photoOriginSelImage = [UIImage sp_imageNamedFromMyBundle:photoOriginSelImageName];
}
...
- (void)configDefaultBtnTitle {
    self.doneBtnTitleStr = [NSBundle sp_localizedStringForKey:@"Done"];
    self.cancelBtnTitleStr = [NSBundle sp_localizedStringForKey:@"Cancel"];
    self.previewBtnTitleStr = [NSBundle sp_localizedStringForKey:@"Preview"];
    self.fullImageBtnTitleStr = [NSBundle sp_localizedStringForKey:@"Full image"];
    self.settingBtnTitleStr = [NSBundle sp_localizedStringForKey:@"Setting"];
    self.processHintStr = [NSBundle sp_localizedStringForKey:@"Processing..."];
}
...
- (void)setShowSelectedIndex:(BOOL)showSelectedIndex {
    ...
    [SPImagePickerConfig sharedInstance].showSelectedIndex = showSelectedIndex;
}

- (void)setShowPhotoCannotSelectLayer:(BOOL)showPhotoCannotSelectLayer {
    _showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
    [SPImagePickerConfig sharedInstance].showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
}

- (void)setNotScaleImage:(BOOL)notScaleImage {
    _notScaleImage = notScaleImage;
    [SPImagePickerConfig sharedInstance].notScaleImage = notScaleImage;
}

- (void)setNeedFixComposition:(BOOL)needFixComposition {
    _needFixComposition = needFixComposition;
    [SPImagePickerConfig sharedInstance].needFixComposition = needFixComposition;
}
...
- (void)observeAuthrizationStatusChange {
    ...
    if ([[SPImageManager manager] authorizationStatusAuthorized]) {
        ...
        SPAlbumPickerController *albumPickerVc = (SPAlbumPickerController *)self.visibleViewController;
        if ([albumPickerVc isKindOfClass:[SPAlbumPickerController class]]) {
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        SPPhotoPickerController *photoPickerVc = [[SPPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[SPImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage needFetchAssets:NO completion:^(SPAlbumModel *model) {
...
- (UIAlertController *)showAlertWithTitle:(NSString *)title {
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:[NSBundle sp_localizedStringForKey:@"OK"] style:UIAlertActionStyleDefault handler:nil]];
...
- (void)setCircleCropRadius:(NSInteger)circleCropRadius {
    ...
    self.cropRect = CGRectMake(self.view.sp_width / 2 - circleCropRadius, self.view.sp_height / 2 - _circleCropRadius, _circleCropRadius * 2, _circleCropRadius * 2);
}
...
- (void)setCropRect:(CGRect)cropRect {
    ...
    _cropRectLandscape = CGRectMake((self.view.sp_height - widthHeight) / 2, cropRect.origin.x, widthHeight, widthHeight);
}
...
- (void)setPickerDelegate:(id<SPImagePickerControllerDelegate>)pickerDelegate {
    _pickerDelegate = pickerDelegate;
    [SPImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [SPImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}

- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [SPImageManager manager].photoWidth = photoWidth;
}
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (PHAsset *asset in selectedAssets) {
        SPAssetModel *model = [SPAssetModel modelWithAsset:asset type:[[SPImageManager manager] getAssetType:asset]];
...
- (void)setAllowPickingImage:(BOOL)allowPickingImage {
    ...
    [SPImagePickerConfig sharedInstance].allowPickingImage = allowPickingImage;
...
- (void)setAllowPickingVideo:(BOOL)allowPickingVideo {
    ...
    [SPImagePickerConfig sharedInstance].allowPickingVideo = allowPickingVideo;
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    [SPImagePickerConfig sharedInstance].preferredLanguage = preferredLanguage;
    [self configDefaultBtnTitle];
}

- (void)setLanguageBundle:(NSBundle *)languageBundle {
    _languageBundle = languageBundle;
    [SPImagePickerConfig sharedInstance].languageBundle = languageBundle;
    [self configDefaultBtnTitle];
}
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [SPImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
- (void)addSelectedModel:(SPAssetModel *)model {
    [_selectedModels addObject:model];
    [_selectedAssetIds addObject:model.asset.localIdentifier];
    if(self.getCurrentChangeAsset){
        self.getCurrentChangeAsset(YES, model);
    }
}

- (void)removeSelectedModel:(SPAssetModel *)model {
    [_selectedModels removeObject:model];
    [_selectedAssetIds removeObject:model.asset.localIdentifier];
    if(self.getCurrentChangeAsset){
        self.getCurrentChangeAsset(NO, model);
    }
}
...
- (void)viewDidLayoutSubviews {
    ...
    _progressHUD.frame = CGRectMake(0, progressHUDY, self.view.sp_width, self.view.sp_height - progressHUDY);
    _HUDContainer.frame = CGRectMake((self.view.sp_width - 120) / 2, (_progressHUD.sp_height - 90 - progressHUDY) / 2, 120, 90);
...
- (void)callDelegateMethod {
    if ([self.pickerDelegate respondsToSelector:@selector(sp_imagePickerControllerDidCancel:)]) {
        [self.pickerDelegate sp_imagePickerControllerDidCancel:self];
    }
...
@interface SPAlbumPickerController ()<UITableViewDataSource,UITableViewDelegate> {
    UITableView *_tableView;
}
@property (nonatomic, strong) NSMutableArray *albumArr;
@end

@implementation SPAlbumPickerController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (imagePickerVc.allowPickingImage) {
        self.navigationItem.title = [NSBundle sp_localizedStringForKey:@"Photos"];
    } else if (imagePickerVc.allowPickingVideo) {
        self.navigationItem.title = [NSBundle sp_localizedStringForKey:@"Videos"];
    }
    
    if (self.isFirstAppear && !imagePickerVc.navLeftBarButtonSettingBlock) {
        self.navigationItem.backBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:[NSBundle sp_localizedStringForKey:@"Back"] style:UIBarButtonItemStylePlain target:nil action:nil];
    }
...
- (void)configTableView {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        SPImagePickerController *imagePickerVc = (SPImagePickerController *)self.navigationController;
        [[SPImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage needFetchAssets:!self.isFirstAppear completion:^(NSArray<SPAlbumModel *> *models) {
            dispatch_async(dispatch_get_main_queue(), ^{
                self->_albumArr = [NSMutableArray arrayWithArray:models];
                for (SPAlbumModel *albumModel in self->_albumArr) {
                ...
                if (!self->_tableView) {
                    ...
                    [self->_tableView registerClass:[SPAlbumCell class] forCellReuseIdentifier:@"TZAlbumCell"];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat naviBarHeight = self.navigationController.navigationBar.sp_height;
    BOOL isStatusBarHidden = [UIApplication sharedApplication].isStatusBarHidden;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (!isStatusBarHidden) top += [SPCommonTools sp_statusBarHeight];
        tableViewHeight = self.view.sp_height - top;
    } else {
        tableViewHeight = self.view.sp_height;
    }
    _tableView.frame = CGRectMake(0, top, self.view.sp_width, tableViewHeight);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    SPAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:@"TZAlbumCell"];
    SPImagePickerController *imagePickerVc = (SPImagePickerController *)self.navigationController;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    SPPhotoPickerController *photoPickerVc = [[SPPhotoPickerController alloc] init];
    photoPickerVc.columnNumber = self.columnNumber;
    SPAlbumModel *model = _albumArr[indexPath.row];
    photoPickerVc.model = model;
...
@implementation UIImage (SPMyBundle)

+ (UIImage *)sp_imageNamedFromMyBundle:(NSString *)name {
    NSBundle *imageBundle = [NSBundle sp_imagePickerBundle];
...
+ (CGFloat)sp_statusBarHeight {
    return [self sp_isIPhoneX] ? 44 : 20;
}

// 获得Info.plist数据字典
+ (NSDictionary *)sp_getInfoDictionary {
...
@implementation SPImagePickerConfig

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static SPImagePickerConfig *config = nil;
    dispatch_once(&onceToken, ^{
        if (config == nil) {
            config = [[SPImagePickerConfig alloc] init];
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    _languageBundle = [NSBundle bundleWithPath:[[NSBundle sp_imagePickerBundle] pathForResource:preferredLanguage ofType:@"lproj"]];
}
