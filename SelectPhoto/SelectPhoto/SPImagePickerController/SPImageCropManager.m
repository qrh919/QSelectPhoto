...
@implementation SPImageCropManager
...
/// 获得裁剪后的图片 rect 外界需要裁剪的尺寸
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView scale:(CGFloat)scale{
    if(scale != 1){//需重新计算尺寸
        ...
        CGFloat xMargin = 0;//containerView.sp_width - CGRectGetMaxX(rect) - rect.origin.x;
        ...
        transform = CGAffineTransformTranslate(transform, translation.x, translation.y/scale);
        // 缩放的处理
        transform = CGAffineTransformScale(transform, zoomScale, zoomScale);
        CGFloat width = rect.size.width / scale;
        CGFloat height = rect.size.height / scale;
        CGFloat imgWidth = imageView.frame.size.width / scale;
        CGFloat imgHeight = imageView.frame.size.height / scale;
        
        CGImageRef imageRef = [self newTransformedImage:transform
                                            sourceImage:imageView.image.CGImage
                                             sourceSize:imageView.image.size
                                            outputWidth:width * [UIScreen mainScreen].scale
                                               cropSize:CGSizeMake(width, height)
                                          imageViewSize:CGSizeMake(imgWidth, imgHeight)];
        UIImage *cropedImage = [UIImage imageWithCGImage:imageRef];
        cropedImage = [[SPImageManager manager] fixOrientation:cropedImage];
        
        CGImageRelease(imageRef);
        return cropedImage;
    }else{
        return [self cropImageView:imageView toRect:rect zoomScale:zoomScale containerView:containerView];
    }
}
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    CGRect imageViewRect = [imageView convertRect:imageView.bounds toView:containerView];
    CGPoint point = CGPointMake(imageViewRect.origin.x + imageViewRect.size.width / 2, imageViewRect.origin.y + imageViewRect.size.height / 2);
    CGFloat xMargin = containerView.sp_width - CGRectGetMaxX(rect) - rect.origin.x;
    CGPoint zeroPoint = CGPointMake((CGRectGetWidth(containerView.frame) - xMargin) / 2, containerView.center.y);
    CGPoint translation = CGPointMake(point.x - zeroPoint.x, point.y - zeroPoint.y);
...
+ (UIImage *)sd_sp_animatedGIFWithData:(NSData *)data {
    if (!data) {
        return nil;
    }
    ...
    if (count <= 1) {
        ...
        NSInteger maxCount = [SPImagePickerConfig sharedInstance].gifPreviewMaxImagesCount ?: 50;
